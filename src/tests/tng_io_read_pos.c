...
int main(int argc, char** argv)
{
    ...
    if (tng_atom_name_of_particle_nr_get(traj, particle, atom_name, sizeof(atom_name)) == TNG_SUCCESS
        ...
        && tng_chain_name_of_particle_nr_get(traj, particle, chain_name, sizeof(chain_name)) == TNG_SUCCESS)
    {
    ...
    if (tng_particle_data_interval_get(traj, TNG_TRAJ_POSITIONS, first_frame, last_frame, TNG_USE_HASH,
                                       &positions, &n_particles, &n_values_per_frame, &data_type)
        == TNG_SUCCESS)
    {
                ...
                for (j = 0; j < n_values_per_frame; j++)
                {
                    switch (data_type)
                    {
                        case TNG_INT_DATA:
                            ...
                            break;
                        case TNG_FLOAT_DATA: printf("\t%f", positions[i][particle][j].f); break;
                        case TNG_DOUBLE_DATA: printf("\t%f", positions[i][particle][j].d); break;
                        default: break;
                    }
                    printf("\n");
                }
    ...
    if (positions)
    {
        tng_particle_data_values_free(traj, positions, n_frames, n_particles, n_values_per_frame, data_type);
    }
    tng_trajectory_destroy(&traj);
